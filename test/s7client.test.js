...
describe("S7Client tests", function () {
  ...
  describe("Constructor", () => {
    it("should create a s7client instance", () =>
      expect(client).to.be.an.instanceof(S7Client));
  });

  describe("Connect", async () => {
    it("should return with CPUInfo object", async () =>
      expect(await client.connect()).to.be.equal(true));
  });

  describe("writeVar", async () => {
    it("should resolve with an object", async () =>
      expect(
        await client.writeVar({
          area: "db",
          type: "CHAR",
          dbnr: 1,
          start: 1,
  ...
  describe("dnsLookup", async () => {
    it("should be a static function", () =>
      expect(S7Client.dnsLookup).to.be.a("function"));
    it("should resolve an ip", async () =>
      expect(await S7Client.dnsLookup("localhost")).to.be.a("string"));
  });

  describe("isConnected", () => {
    it("should return the connection state", () =>
      expect(client.isConnected()).to.be.a("boolean"));
  });

  describe("getCpuInfo", async () => {
    it("should return an object", async () =>
      expect(await client.getCpuInfo()).to.be.a("object"));
  });
  ...
  describe("autoconnect", async () => {
    it("should return with CPUInfo object", async () =>
      expect(await client.autoConnect()).to.be.equal(true));
  });
