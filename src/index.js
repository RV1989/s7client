...
class S7Client extends EventEmitter {
  ...
  async connect() {
    if (this.isConnected())
      return Promise.reject(
        new Error(`Already connected to ${this.opts.name}`)
      );
    debug(
      `Connecting to ${this.opts.name} on ${this.opts.host}:${this.opts.port}, Rack=${this.opts.rack} Slot=${this.opts.slot}`
    );

    return (isIPv4(this.opts.host)
      ? Promise.resolve(this.opts.host)
      : S7Client.dnsLookup(this.opts.host)
    ).then(
      (ip) =>
        new Promise((resolve, reject) => {
          this.client.ConnectTo(ip, this.opts.rack, this.opts.slot, (err) => {
            ...
            this.emit("connect", this.isConnected());
  ...
  async autoConnect() {
    ...
    function _retry() {
      ...
      self.connect().catch(() => {
        ...
        if (retryDelay > self.opts.maxRetryDelay)
          retryDelay = self.opts.maxRetryDelay;
  ...
  async readDB(DBNr, vars) {
    return new Promise((resolve, reject) => {
      ...
      debug(
        `${this.opts.name}: ReadDB DB=${DBNr}, Offset=${offset}, Length=${
          end - offset
        }`
      );
  ...
  async readVars(vars) {
    return new Promise((resolve, reject) => {
      ...
      this.client.ReadMultiVars(toRead, (err, res) => {
        ...
        res = vars.map((v, i) => {
          if (res[i].Result !== 0)
            return errs.push(this.client.ErrorText(res[i].Result));
  ...
  async readVar(v) {
    if (v.area === "db" && !v.dbnr)
      throw new Error(`Param dbnr is mandatory for area=db`);
    return this.readVars([v]).then((r) => r[0]);
  }
   ...
   */
  _getErr(s7err) {
    if (Array.isArray(s7err))
      return new Error(`${this.opts.name} Errors: ` + s7err.join("; "));
